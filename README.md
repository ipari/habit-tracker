...
- 설정 화면의 IANA 시간대 변경과 현재 기기 알림 연결 해제
...
설정 화면에서 IANA 시간대를 변경하면 날짜 계산은 즉시 새 시간대를 사용합니다. 기존 습관 알림도 새 시간대에서 저장된 현지 시각을 그대로 유지합니다. `연결 해제`는 현재 기기의 Push Subscription만 비활성화하며 브라우저 자체의 알림 권한은 변경하지 않습니다.

오늘 또는 습관 목록에서 습관 상세 화면을 연 뒤 `성과 공유`를 누르면 선택한 배경 프리셋과 현재 streak로 1080×1920 이미지를 만듭니다. 브라우저가 파일 공유를 지원하면 시스템 공유 시트를 열고, 지원하지 않거나 공유에 실패하면 PNG 다운로드를 사용할 수 있습니다. 이미지에는 로그인 아이디나 서비스 주소를 넣지 않습니다.
