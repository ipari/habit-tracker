...
새 습관은 기본 알림 시간 `09:00`과 활성화된 `알림 받기`로 시작합니다. 알림 요일은 항상 습관의 수행 요일을 따라갑니다. 알림 시간을 지정하거나 변경하면 `알림 받기`가 자동으로 켜지며, 이후 원하면 다시 끌 수 있습니다. 습관을 보관하면 해당 알림도 비활성화되고, 다시 시작할 때 저장된 시간이 있으면 알림도 다시 활성화됩니다. 별도 스케줄러 프로세스가 현지 시간과 IANA 시간대를 기준으로 활성 기기에 Web Push를 발송하며, 만료된 구독은 자동으로 비활성화합니다.
