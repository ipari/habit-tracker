...
새 습관의 `시간 설정`은 기본적으로 꺼져 있습니다. 이를 켜면 기본 시간 `09:00`과 `알림 받기`가 활성화되며, 시간을 저장한 뒤에도 알림만 별도로 끌 수 있습니다. 알림 요일은 항상 습관의 수행 요일을 따라갑니다. 습관을 보관하면 해당 알림도 비활성화되고, 다시 시작할 때 저장된 시간이 있으면 알림도 다시 활성화됩니다. 별도 스케줄러 프로세스가 현지 시간과 IANA 시간대를 기준으로 활성 기기에 Web Push를 발송하며, 만료된 구독은 자동으로 비활성화합니다.
