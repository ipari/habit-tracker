...
- 오늘·습관·달력 목록의 연속 달성·수행 요일·선택적으로 지정한 12시간제 시간 요약
- 오늘·달력 목록의 미달성 우선 및 알림 시간 오름차순 정렬
- 진행 중인 습관과 접을 수 있는 `지난 습관들` 영역
- 목록에서 습관 상세로 이동해 수정·공유·보관하고, 보관된 습관을 다시 시작하는 관리 흐름
- 홈 또는 습관들 중 실제 진입 화면을 유지하는 상세·수정 뒤로가기
...
- 습관별 내장 그라데이션 공유 배경 프리셋 선택
- 현재 streak와 습관 시작일을 담은 1080×1920 PNG 미리보기, 아이콘 버튼을 통한 시스템 공유 및 다운로드
...
- 알림 권한 위에 배치된 우측 정렬 아이콘 전용 화면 모드 선택 컨트롤
- 좌우 대칭 스크롤바 여백과 스크롤 가능한 화면에서만 표시되는 투명 트랙의 스크롤 막대
- 홈 화면에 설치 가능한 PWA와 정적 자산 캐시
- 투명 외곽의 흰 라운드 타일 안에 앱의 완료 상태와 같은 초록 원·흰 체크를 사용하는 PWA·Dock 앱 아이콘 및 작은 크기에 맞춰 심볼을 확대한 favicon
- 시스템 alert·confirm 대신 반투명 배경과 포커스 차단을 적용한 앱 내 공통 알림 모달
...
새 습관의 `시간 설정`은 기본적으로 꺼져 있습니다. 이를 켜면 기기의 현재 시각을 10분 단위로 내림한 시간과 `알림 받기`가 활성화됩니다. 예를 들어 12:34에는 12:30이 입력됩니다. 시간을 저장한 뒤에도 알림만 별도로 끌 수 있으며, `시간 설정`을 다시 끄고 저장하면 시간과 알림 설정이 함께 제거됩니다. 알림 요일은 항상 습관의 수행 요일을 따라갑니다. 습관을 보관하면 해당 알림도 비활성화되고, 다시 시작할 때 저장된 시간이 있으면 알림도 다시 활성화됩니다. 별도 스케줄러 프로세스는 기본 30초마다 최근 5분의 발송 시점을 확인해 활성 기기에 Web Push를 전송합니다. 일시적 실패는 기기별로 최대 3회 재시도하고, 만료된 구독은 자동으로 비활성화합니다.
...
운영 `.env`는 애플리케이션이 권한을 자동 변경하지 않으므로 직접 `chmod 600 .env`를 실행해 보호합니다. 비밀번호를 변경하려면 새 해시를 생성해 `HABIT_TRACKER_PASSWORD_HASH`를 교체하고 앱을 재시작합니다. 기존 로그인 세션은 새 해시와 일치하지 않아 자동으로 무효화됩니다.
...
오늘 또는 습관 목록에서 습관 상세 화면을 연 뒤 `공유`를 누르면 선택한 그라데이션 프리셋, 현재 streak, 습관 시작일로 브라우저 Canvas에서 1080×1920 이미지를 만듭니다. 이모지가 없으면 `✨`를 사용하고 이미지 하단에는 `HABIT TRACKER` 워드마크를 표시합니다. 상세 화면에는 첫 시작일, 요일, 저장된 시간, 알림 켜짐 여부를 표시합니다. `보관`은 과거 달성 기록과 일정을 지우지 않고 보관 당일부터 오늘·달력 목록에서 숨기며 알림을 끕니다. 보관일 이전의 달력에는 과거 기록이 계속 표시됩니다. 확인창에는 기록과 일정이 유지되고 보관한 습관을 언제든 다시 시작할 수 있음을 안내합니다. 보관된 습관의 `다시 시작`을 누르면 마지막 수행 요일로 재개하고 저장된 시간이 있으면 알림도 다시 켭니다. 브라우저가 파일 공유를 지원하면 시스템 공유 시트를 열고, 지원하지 않거나 공유에 실패하면 PNG 다운로드를 사용할 수 있습니다. 이미지에는 로그인 아이디나 서비스 주소를 넣지 않습니다.
...
현재 Compose에는 자동 백업 서비스가 없습니다. 실행 중인 DB 파일을 단순 복사하지 말고 운영 환경에서 SQLite Online Backup API 또는 `VACUUM INTO`를 사용하는 별도 백업 작업과 복구 절차를 구성해야 합니다. `restart: unless-stopped`는 Docker 데몬이 실행된 뒤의 컨테이너 재시작만 담당하므로 Windows/WSL2 부팅 시 Docker와 Compose 스택을 시작하는 설정도 호스트에서 별도로 준비합니다.
